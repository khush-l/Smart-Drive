...
def generate_enhanced_instruction(html_instruction, lat, lng, model_name="gpt-3.5-turbo"):
    ...
    if is_in_hotspot(lat, lng):
        return "High‑crash zone: drive slowly"

    # Otherwise, ask the LLM to rephrase
    prompt = (
        "Rephrase this navigation instruction into a concise, TTS‑friendly cue "
        "(under 15 words):\n\n"
        f"{html_instruction}"
    )
...
def main():
    ...
    if api_key == "your_api_key_here":
        raise ValueError("Please replace placeholder API key with your actual key.")
    ...
    print(f"Safest Route: {safest_route_idx + 1} with Safety Score {safety_scores_and_times[safest_route_idx][0]:.2f}/10")
